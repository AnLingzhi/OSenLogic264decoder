...
unsigned short **intra_pred_mode; // 4x4块的9种模式 4x4块为单位 只对宏块采用4x4方式时有用,
unsigned short **intra_mode; // 1 : intra, 0 : inter 16x16块为单位
unsigned short **qp_z;
...
int main(int argc, char* argv[])
{
	...
	// 除了最底层对位进行操作的函数,其余函数bytes_offset,bits_offset都为引用传递,
...
void InitDecoder()
{
	...
	get_mem2Dpixel(&RefIdx, HEIGHT * 2, WIDTH * 2, -1);//8x8为单位
...
{
	unsigned short i, j;	
	int k; /* WIDTH * HEIGHT * 128 * 3的范围超过了unsigned short能表示的范围  */
...
{
	unsigned i;
	// 因为是unsigned short类型，每个unsigned short都占两个字节，memset只set了一半的内存
